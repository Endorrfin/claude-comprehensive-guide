...
> **Build status (updated 2026-06-23, through S10a).** Authored ✅ = M1–M27 (**27 / 28**). Sections **I, II, III,
> IV & V complete + Section VI M25–M27** (S10a). **All 7 signature sims built** (Tool Picker = the last, S10a;
> `#/decide` is live). Only **M28** (mental-models gallery + glossary + cheat-sheet + flashcards) remains —
> deferred to **S10b** with the perf + bilingual-QA polish. This file is the **seed plan**; where verification
> refined a topic, the authored content + the correction note live in `concepts.ts` and `CLAUDE.md` §14 (e.g. M4
> chat uploads are **500 MB/file** not 30 MB, and writing **styles are migrating to Skills**; M2 has **no official
> keyboard-shortcut sheet**; M5 Memory is **two spaces** — global + walled per-project — and chat search is
> **paid-only**; M19 connectors & plugins are **account-level, not project-scoped**; M21 "shared context" = the
> separate **Work across Microsoft 365 apps** feature, Office agents **GA on all paid plans**; M20 Chrome on Pro =
> Haiku-only; **M25 access-tier names (View/Click/Full) are documented for Claude Code Desktop, and the ~1%
> injection-robustness figure is Opus-4.5/Nov-2025-specific**).
